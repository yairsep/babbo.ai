...
assert.equal((await call('/community','GET',undefined,b)).status,403);
const dad1Community=await call('/community/profile','PUT',{display_name:'Tom',area:'Amsterdam Oost',intro:'New here, up for a coffee walk.',visible:true,confirmed:true},a);
assert.equal(dad1Community.status,200,JSON.stringify(dad1Community.data));
const dad2Community=await call('/community/profile','PUT',{display_name:'Ravi',area:'Amsterdam West',visible:true,confirmed:true},b);
assert.equal(dad2Community.status,200,JSON.stringify(dad2Community.data));
const directory=await call('/community','GET',undefined,a);
assert.equal(directory.status,200);
assert.equal(directory.data.dads.length,1);
assert.equal(directory.data.dads[0].display_name,'Ravi');
assert.equal(directory.data.dads.some(d=>d.display_name==='Tom'),false);
assert.equal(JSON.stringify(directory.data).includes('@example.com'),false);
assert.equal((await call('/community/profile','DELETE',{confirmed:true},b)).status,200);
assert.equal((await call('/community/profile','GET',undefined,b)).data.community_profile,null);
...
console.log('Integration passed: registration, onboarding, guidance, confirmed recurring action, Today data, isolation, completion, community directory, deletion.');
